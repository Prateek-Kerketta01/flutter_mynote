...
void main() {
  group('Mock Authenticatoin', () {
    ...
    test('Canot lgo out if not initialized', () {
      expect(
        provider.logout(),
    ...
    test('Create user should delegate to logIn function', () async {
      final badEmailUser =
          provider.createUse(email: 'foo@bar.com', password: 'anypassword');
      expect(badEmailUser,
          throwsA(const TypeMatcher<UserNotFoundAuthException>()));

      final badPasswordUser =
          provider.createUse(email: 'someone@bar.com', password: 'foobar');
      expect(badPasswordUser,
          throwsA(const TypeMatcher<WrongPasswordAuthException>()));

      final user = await provider.createUse(
        email: 'foo',
        password: 'bar',
      );
